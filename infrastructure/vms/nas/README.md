...
| `tank/paperless` | `/mnt/tank/paperless` | Paperless-ngx consume + export |
| `tank/gitlab` | `/mnt/tank/gitlab` | GitLab repository backups |

### 4. Create SMB Shares

Sharing → Windows Shares (SMB) → Add:
- Path: `/mnt/tank/paperless`
- Name: `paperless`
- Enable: ✓
...
On lab-paperless01, mount the NAS SMB consume share:
```bash
mount -t cifs //10.10.10.70/paperless /mnt/nas-paperless -o username=paperless,password=yourpass
```

### GitLab → NAS

Configure GitLab backups to write to `/mnt/tank/gitlab`:
```yaml
# In docker-compose.yml GITLAB_OMNIBUS_CONFIG
gitlab_rails['backup_path'] = '/var/opt/gitlab/backups'
# Map this inside the container to the NAS mount:
# volumes: - /mnt/nas-gitlab:/var/opt/gitlab/backups
```
